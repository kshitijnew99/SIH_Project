...
const SimpleLeafletMap: React.FC<SimpleLeafletMapProps> = ({ 
  ...
  useEffect(() => {
    if (mapRef.current && !leafletMapRef.current) {
      ...
      // Add initial marker
      markerRef.current = L.marker([center.lat, center.lng], { icon: redIcon })
        .addTo(leafletMapRef.current)
        .bindPopup(`<div class="text-center"><strong>Selected Location</strong><br/>${center.lat.toFixed(6)}, ${center.lng.toFixed(6)}</div>`);
  ...
  useEffect(() => {
    if (leafletMapRef.current && markerRef.current) {
      ...
      markerRef.current.setPopupContent(`<div class="text-center"><strong>Selected Location</strong><br/>${selectedLocation.lat.toFixed(6)}, ${selectedLocation.lng.toFixed(6)}</div>`);
